...
* 关于菜品识别API：百度的菜品识别API还在不断迭代，目前的识别误差偏大，很难识别出流质的食物，如咖喱，汤等，以及特征较为模糊的食物。并且菜品库中的数据较少，一些比较少见的菜无法进行识别，仍需不断扩充数据库。价格在目前的市场中因为无同类相似竞品，无法进行比较，也一定程度上的垄断了这一美食识别搜索的细分领域。

* 关于图片对比度增强API：图片的对比度增强效果还是需要进一步优化，目前的优化效果化石更趋于对图片亮度的提高，其他的一些对比度增强还是没有十分显著
